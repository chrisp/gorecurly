...
func TestLive(t *testing.T) {
    ...
    log.Println("create valid account with billing info")
    acc2 := r.NewAccount()
    rand.Seed(int64(time.Now().Nanosecond()))
    rvalue = fmt.Sprintf("%v", rand.Intn(400000))
    ...
    log.Println("create valid account with billing info for invoicing")
    acc4 := r.NewAccount()
    rand.Seed(int64(time.Now().Nanosecond()))
    rvalue = fmt.Sprintf("%v", rand.Intn(400000))
    ...
    log.Println("close account")
    if closeerr := getacc.Close(); closeerr != nil {
        t.Fatal(err.Error())
    }

    log.Println("Get closed account and check state")
    getacc, err = r.GetAccount(acc2.AccountCode)
    if err != nil {
        t.Fatal(err.Error())
    }
